...
async def main():
    ...
    async with SessionLocal() as session:
        ...
        delete_stmt = delete(IngestedDocument).where(
            IngestedDocument.source_identifier.in_(urls)
        )
        res = await session.execute(delete_stmt)
        await session.commit()

        print(
            f"[SUCCESS] Cleaned metadata. IngestedDocument records deleted: {res.rowcount}"
        )
