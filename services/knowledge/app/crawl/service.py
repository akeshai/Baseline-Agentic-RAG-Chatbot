...
class CrawlService:
    ...
    async def run_crawl_background(
        ...
        """
        logger.info("Background crawl task ID %s waiting for semaphore...", task_id)

        async with _concurrent_crawls_semaphore:
            logger.info("Background crawl task ID %s started.", task_id)
            ...
            try:
                ...
                logger.exception(
                    "Crawl task ID %s failed in background execution", task_id
                )
                status = "failed"
                error_message = str(e)
