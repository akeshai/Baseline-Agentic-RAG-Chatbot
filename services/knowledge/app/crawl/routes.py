...
async def create_crawl_task(
    ...
    return await CrawlService.start_crawl_task(
        db, current_user.user_id, crawl_req, background_tasks
    )
...
async def list_crawl_tasks(
    ...
    """
    return await CrawlRepository.list_tasks_by_user(db, current_user.user_id)
...
async def get_crawl_task(
    ...
    """
    task = await CrawlRepository.get_task_by_id(db, task_id, current_user.user_id)
    if not task:
        raise HTTPException(
...
async def get_page_detail(
    ...
    task = await CrawlRepository.get_task_by_id(db, page.task_id, current_user.user_id)
    if not task:
        raise HTTPException(
