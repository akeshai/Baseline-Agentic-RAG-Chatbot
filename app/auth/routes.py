...
async def get_current_user(api_key: Optional[str] = Security(api_key_header), db: AsyncSession = Depends(get_db)) -> User:
    """
...
async def create_key(
    ...
    return await AuthService.create_api_key(db, user.id, key_in.name)
...
async def list_keys(
    ...
    """
    return await AuthService.list_api_keys(db, current_user.id)
...
async def revoke_key(
    ...
    """
    await AuthService.revoke_api_key(db, current_user.id, id)
...
@router.put("/users/{user_id}/role", response_model=UserResponse, dependencies=[Depends(RoleChecker(["admin"]))])
